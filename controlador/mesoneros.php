...
if (isset($_SESSION['usuId'])){
	...
	//Incluir las Clases y Objetos
	include ("../modelo/mesoneros.class.php");
	$objeto = new mesoneros;
...
switch ($oper) {
		...
		case 'agregar':
			...
			if($existe>0){
				...
				echo ('Por favor seleccione un estatus');
			  	...
			  	$data = explode("/",$fechan,3);
			    $fechanX = $data[2].'-'.$data[1].'-'.$data[0];
		...
		case 'autocompletar_parro':
			
				$where=" where parrDescripcionC like '%".$term."%' or muniDescripcionC like '%".$term."%' or edoDescripcionC like '%".$term."%'";
				$enti = 'parroquia
						INNER JOIN municipio ON parroquia.parrMuniIdC = municipio.muniIdC
						INNER JOIN estado ON municipio.muniEdoIdC = estado.edoIdC';
				$accion = $objetoG->consultaGeneral($enti, $where, 'parrDescripcionC', 'asc', 0, 10);
				$responce='';
				foreach($accion as $row){
				  $responce[] = array(
						'label'    	  => $row['parrDescripcionC'].' Municipio: '.$row['muniDescripcionC'].' Estado: '.$row['edoDescripcionC'],
						'value'    	  => $row['parrDescripcionC'].' Municipio: '.$row['muniDescripcionC'].' Estado: '.$row['edoDescripcionC'],
						'idparroo'    => $row['parrIdC'],
						'evaluo'      => 1
					);
				 }
				  if (count($accion) == 0) {
					$responce[] = array(
						...
						'label'    	  => $term .' Parroquia no encontrada',
						'value'    	  => $term ,
						'idparroo'    => '0-0-0',
						'evaluo'      => 0
					);
