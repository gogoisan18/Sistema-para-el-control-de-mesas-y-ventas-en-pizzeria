...
session_start();
	...
	include ("../modelo/carnes.class.php");
	$objeto = new carnes;
...
switch ($oper) {
			...
			case 'agregar':
				...
				$existe = $objetoG->cantidadReg($entidad," Where carnesDescripC = '" . $descrix . "' ");
			...
			case 'buscar_infor':
				  ...
				  $csql="SELECT * from carnes  WHERE carnesIdE = '".$term."' ";
				  ...
				  if(count($accion)>0){
						foreach($accion as $row){
							...
							$salida = (int)$row['carnesIdE'].'||'.$row['carnesDescripC'].'||'.$row['carnesPrecioD'].'||'.$row['carnesTipoE'];
					    }
				  }
				echo $salida;
            ...
            default:
					...
					$where=" where carnesEstatusE='Disponible'";
					...
					foreach ($registros as $row) {
						...
						$responce->rows[$i]['id'] = $row['carnesIdE'];
