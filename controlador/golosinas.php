...
session_start();
	...
	include ("../modelo/golosinas.class.php");
	$objeto = new golosinas;
...
switch ($oper) {
			...
			case 'agregar':
				...
				$existe = $objetoG->cantidadReg($entidad," Where descrip = '" . $descrix . "' ");
              				 
				if($existe>0){
					echo ('Ya existe un registrado con este nombre');
				}else{
			...
			case 'buscar_infor':
				  ...
				  $csql="SELECT * from golosinas  WHERE id = '".$term."' ";
				  ...
				  if(count($accion)>0){
						foreach($accion as $row){
							...
							$salida = (int)$row['id'].'||'.$row['descrip'].'||'.$row['precio'];
					    }
				  }
				echo $salida;
            ...
            default:
					...
					$where=" where estatus ='Disponible' ";
